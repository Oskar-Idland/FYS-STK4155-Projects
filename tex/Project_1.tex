...
\begin{figure*}
  ...
  \caption{MSE evaluated on Franke test data as function of polynomial degrees in the interval $[1, 11]$ and number of folds $k\in[5, 15]$ used in cross-validation, for OLS (first row), Ridge (second row) and Lasso (third row) regression. The upper left contour plot\colorbox{magenta}{update after running cross-val again}. We have used $\lambda=0.001$ and $\lambda=0.1$ in the left and right columns, respectively, for both Ridge and Lasso regression. \colorbox{magenta}{try using 0.001 and 5 instead?}}\label{fig:f OLS Ridge Lasso}
  \vspace*{-5pt}
\end{figure*}
