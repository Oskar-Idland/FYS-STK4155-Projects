...
To showcase the bias-variance trade-off and validate our implementation, we imitated the analysis illustrated in \cref[fig:Hastie]{Fig. 2.11 from Hastie, Tibshirani, and Friedman}. Specifically, we implemented OLS regression for polynomial degrees $\in[1,81]$ in steps of 4, and computed the MSE evaluated on both the training and test portions of the Franke data each time. The generation and splitting of the Franke data were conducted as described previously. \\
