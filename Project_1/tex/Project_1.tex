...
\begin{equation}
  a = \frac{1}{1+z}.
\end{equation}
Here $z$ is the cosmological redshift, which is 0 today and thus in the 128th snapshot. We focus on the dark matter density $\rho$, and study a snapshot at redshift $z\approx12.88$ because the distribution of $\rho$ is smoother at larger $z$. To get a function of two variables we average over one of the three axes so that we get a 2D grid, and because the density is so large and vastly different at different points in the box we take $\ln\rho$ to be our dependent variable instead of $\rho$. The resulting data is visualized as a contour plot and a surface plot in figure \ref{fig:density}. \colorbox{magenta}{maybe remove figure}
...
\begin{figure*}
  ...
  \caption{Logarithm of dark matter density in the simulation box averaged over one of the three axes at redshift $z\approx12.88$. Plotted as contour plot on the left and as surface plot on the right.}\label{fig:density}
  \vspace*{-5pt}
\end{figure*}
...
The Franke function generated data used in this study was created by sampling the function on a grid of $N\times N$ points, where $N=50$. The data was then augmented with an gaussian noise component with a zero mean and standard deviation of $\sigma = 0.1$. To match the $N\times N$ grid of the data,  the noise was flattened into a $(N^2, 1)$ array. The noise was added to simulate the inherent variability and uncertainty in real-world data.

\colorbox{magenta}{Håvard explain cosmo data?}
...
To showcase the bias-variance trade-off and validate our implementation, we imitated the analysis illustrated in \cref[fig:Hastie]{Fig. 2.11 from Hastie, Tibshirani, and Friedman}. Specifically, we plotted the MSE for both the training and test sets of the Franke data using OLS regression. We varied the polynomial degree from 1 to 82 in steps of 4. \colorbox{magenta}{Link to our result?}

The generation and splitting of the Franke data were conducted as described previously. \\

\begin{figure}[h!]
  \centering %Centers the figure
  \includegraphics[width=\columnwidth]{../figs/hastie_bias_variance_tradeoff.jpg}
  \caption{Fig. 2.11 of Hastie, Tibshirani, and Friedman illustrating bias-variance trade-off as a function of model complexity \colorbox{magenta}{cite}}\label{fig:Hastie}
\end{figure}
...
For the Franke function-generated data, we designed four different scenarios, each varying a specific parameter to investigate the influence of the number of bootstraps, data points, model complexity, and test size separately.

The default values for each scenario were set as follows: 100 bootstraps, 50 data points, polynomial degree of 5, and a test size of 20\%. The varying parameters for each respective scenario were:
...
\begin{itemize}
   ...
   \item [-]Test size: ranging from 5\% to 95\% with a step of 5\%
\end{itemize}

For each scenario we plotted the model evaluation metrics as a function of the varying parameter to analyze how the model's performance changed with different settings. 

For the cosmological simulation data, we only did a single scenario with 100 bootstraps, 50 data points, test size of 20\%, and a varying polynomial degree from 1 to 31 with a step of 3. We only plotted the MSE as a function of polynomial degree for this scenario. \colorbox{magenta}{Explain why?}
...
% TODO: Write
For our cross-validation analysis, we used the \verb|KFold| function from the scikit-learn library to split the data into $k$ folds, with the integer $k ∈ [5, 15]$. We iterated over a 
...
All source code for this project is available on our GitHub repository, linked in \cref{appsec:code}. The \verb|README.md| contains the entire project structure. The code is divided into the following files and notebooks:

\verb|data_analysis.ipynb|: Jupyter notebook containing the analysis of the dark matter density data. This is where we apply our regression models.

\verb|franke.ipynb|: Jupyter notebook containing the analysis of the Franke function-generated data, to validate our implementation.
...
Python \cite{Python} is used for creating our models, together with scikit-learn \cite{scikit-learn}. To vectorize our code we use \verb|numpy| \cite{Numpy}, and for visualization we use \verb|matplotlib.pyplot| \cite{Matplotlib}. To make our plots prettier we use the colormaps module \cite{colormaps}. Code completion and debugging is done in Visual Studio Code \cite{VSCode} with additional assistance of GitHub Copilot \cite{Copilot}. We use \verb|git| \cite{Git} for version control, and \verb|GitHub| \cite{GitHub} for remote storage of our code.
...
\subsection{Effects of Data Scaling}
...
\begin{figure*}
  ...
  \caption{MSE (upper row) and $R^2$ (lower row) evaluated on the cosmological test set using unscaled (left column) and scaled (right column) values, for polynomial degrees in the interval $[1,41]$.}\label{fig:g error scaled vs raw}
  \vspace*{-5pt}
\end{figure*}
...
\begin{figure}[h!]
  ...
  \caption{The coefficients $\beta$ for the cosmological fit estimated using unscaled (purple) and scaled (orange) data, plotted for polynomial degrees in the interval $[1,41]$.}\label{fig:g beta scaled vs raw}
  \vspace*{-5pt}
\end{figure}

\subsection{Hyperparameter in Ridge and Lasso}
...
\begin{figure*}
  ...
  \caption{MSE evaluated on Franke test data as function of polynomial degrees in the interval $[1, 15]$ and number of folds $k\in[5, 15]$ used in cross-validation, for Ridge (upper row) and Lasso (lower row) regression. The upper left contour plot\colorbox{magenta}{update after running cross-val again}. We have used $\lambda=0.001$ and $\lambda=0.5$ in the left and right columns, respectively. \colorbox{magenta}{try using 0.001 and 5 instead?}}\label{fig:f Ridge Lasso}
  \vspace*{-5pt}
\end{figure*}
...
\begin{figure*}
  ...
  \caption{Surface plots of the cosmological simulation data (left) and the OLS fit for polynomial degree 30 (right).}\label{fig:density surf}
  \vspace*{-5pt}
\end{figure*}
...
\begin{figure*}
  ...
  \caption{MSE evaluated on cosmological test data as function of polynomial degrees in the interval $[1, 31]$ and number of folds $k\in[5, 15]$ used in cross-validation (contour plot on the left). On the right we have plotted the MSE purely as function of complexity when using the bootstrap resampling method with 100 steps.}\label{fig:g kfold vs bootstrap}
  \vspace*{-5pt}
\end{figure*}
...
\begin{figure*}
  ...
  \caption{MSE evaluated on cosmological test data as function of polynomial degrees in the interval $[1, 31]$ and number of folds $k\in[5, 15]$ used in cross-validation, for Ridge (upper row) and Lasso (lower row) regression. The upper left contour plot\colorbox{magenta}{update after running cross-val again}. We have used $\lambda=0.001$ and $\lambda=0.5$ in the left and right columns, respectively. \colorbox{magenta}{try using 0.001 and 5 instead?}}\label{fig:g Ridge Lasso}
  \vspace*{-5pt}
\end{figure*}
...
%   \caption{Contour plots of the cosmological simulation data (upper left) and OLS (upper right), Ridge (lower left) and Lasso (lower right) regression fits for polynomial degree 31 and $k=15$ number of folds in cross-validation. For Ridge regression we used \colorbox{magenta}{lambda} and for Lasso we used \colorbox{magenta}{lambda}.}\label{fig:density OLS Ridge Lasso}
%   \vspace*{-5pt}
% \end{figure*}
...
\end{equation}
Now, using that the OLS estimator is given by $\boldsymbol{\hat{\beta}}=\left(\mathbf{X}^\textrm{T}\mathbf{X}\right)^{-1}\mathbf{X}^\textrm{T}\mathbf{y}$, its expectation values become
\begin{align*}
  \mathbb{E}(\boldsymbol{\hat{\beta}})&=\mathbb{E}\left\{ \left(\mathbf{X}^\textrm{T}\mathbf{X}\right)^{-1}\mathbf{X}^\textrm{T}\mathbf{y} \right\} \\
...
\end{align*}
so if we now set $x=\left(\mathbf{X}^\textrm{T}\mathbf{X}\right)^{-1}\mathbf{X}^\textrm{T}$ and $y=\mathbf{y}$ we find that the variance of $\boldsymbol{\hat{\beta}}$ is
\begin{align*}
